...
def read_file(data_path: str, encoding: str = "utf-8") -> pd.DataFrame:
    """
    ...
    Args:
        ...
        encoding: encoding
    Returns:
        Transactions in pd.DataFrame['date_time', 'customer_id', 'product_id']
    """

    transaction_df = pd.read_csv(data_path, encoding=encoding, header=0)[['TRANSACTION_DT', 'CUSTOMER_ID', 'PRODUCT_ID']]
    transaction_df.columns = ['date_time', 'customer_id', 'product_id']
...
def generate_baskets(transaction_df: pd.DataFrame, items_map_dic_path: str, users_map_dic_path: str):
    ...
    :param transaction_df: pd.DataFrame['date_time', 'customer_id', 'product_id']
    ...
    for user_id, user in tqdm(transaction_df.groupby(['customer_id'])):
        ...
        for day, trans in user.groupby(['date_time']):  # select by user and day
            ...
            basket = Set(user_id, product_index_list, set_time=day.date())
...
if __name__ == "__main__":
    data_path = f"../original_data/TaFeng/ta_feng_all_months_merged.csv"
