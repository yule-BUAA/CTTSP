...
select_days = 60  # select transactions in the first 60 days
...
def read_file(data_path: str):
    ...
    :return:
        transaction_df: pd.DataFrame ['product_id', 'customer_id', 'week', 'day']
    """
    transaction_df = pd.read_csv(data_path, header=0)[['upc', 'household', 'week', 'day']]
    transaction_df.columns = ['product_id', 'customer_id', 'week', 'day']

    # select 60 days
    transaction_df = transaction_df[transaction_df['day'] <= select_days]
...
def generate_baskets(transaction_df: pd.DataFrame, items_map_dic_path: str, users_map_dic_path: str):
    ...
    :param transaction_df: pd.DataFrame ['product_id', 'customer_id', 'week', 'day']
    ...
    for user_id, user in tqdm(transaction_df.groupby(['customer_id'])):
        baskets = []  # [Basket,...]
        for day, trans in user.groupby(['day']):  # select by user and day
            product_index_list = list(set(trans['product_id'].tolist()))
            basket = Set(user_id, product_index_list, set_time=day)
    ...
    print(f'date in the first {select_days} days')
...
if __name__ == "__main__":
    data_path = f"../original_data/dunnhumby-Carbo/dunnhumby-Carbo/dh_transactions.csv"
