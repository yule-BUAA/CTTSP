...
def read_file(data_path: str):
    ...
    Returns:
        Transactions in pd.DataFrame['customer_id', 'product_id', 'date_time', 'type'] in the selected dates
    """
    transaction_df = pd.read_csv(data_path, header=0)[['user_id', 'sku_id', 'action_time', 'type']]
    transaction_df.columns = ['customer_id', 'product_id', 'date_time', 'type']

    # type -> 1 browse, 2 buy, 3 favor, 4 comment, 5 add_to_shopping_cart
    # select buy behavior, with type == 2
    transaction_df = transaction_df[transaction_df['type'] == 2]

    transaction_df['date_time'] = pd.to_datetime(transaction_df['date_time']).astype(str)
    ...
    transaction_df = transaction_df[
        (transaction_df['date_time'] >= start_date) &
        (transaction_df['date_time'] <= end_date)
        ]
...
def generate_baskets(transaction_df: pd.DataFrame, items_map_dic_path: str, users_map_dic_path: str):
    ...
    for user_id, user in tqdm(transaction_df.groupby(['customer_id'])):
        ...
        for day, trans in user.groupby(['date_time']):  # select by user and day
            product_index_list = list(set(trans['product_id'].tolist()))
    ...
    print(f'date start from {start_date}, end at {end_date}')
...
if __name__ == "__main__":
    data_path = f"../original_data/raw_jdata/action.csv"
    ...
    transaction_df = get_frequent_items(transaction_df, frequency_rate=frequency_rate)
